...
namespace Module.General.AutoReferences
{
	...
	{
		private static readonly MethodInfo copyListMethodInfo = typeof(InjectModuleList)
			.GetMethod(nameof(CopyList), BINDING_FLAGS);

		private const BindingFlags BINDING_FLAGS = BindingFlags.Static | BindingFlags.NonPublic;
		...
		public override bool TrySetValueIntoField(object targetObject, FieldInfo fieldInfo, List<object> values)
		{
			...
			fieldInfo.SetValue(targetObject, copyListMethodInfo
		...
		public override bool TrySetValueIntoProperty(object targetObject, PropertyInfo propertyInfo, List<object> values)
		{
			...
			propertyInfo.SetValue(targetObject, copyListMethodInfo
												.MakeGenericMethod(typeArgument)
												.Invoke(this, new object[] { values }));
			return true;
		}
