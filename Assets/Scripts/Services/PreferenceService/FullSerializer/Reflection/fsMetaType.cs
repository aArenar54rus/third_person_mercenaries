...
namespace FullSerializer {
    ...
    public class fsMetaType {
        private static readonly Dictionary<fsConfig, Dictionary<Type, fsMetaType>> configMetaTypes =
            new Dictionary<fsConfig, Dictionary<Type, fsMetaType>>();

        public static fsMetaType Get(fsConfig config, Type type) {
            Dictionary<Type, fsMetaType> metaTypes;
            lock (configMetaTypes)
            {
                if (configMetaTypes.TryGetValue(config, out metaTypes) == false)
                {
                    metaTypes = configMetaTypes[config] = new Dictionary<Type, fsMetaType>();
                }
            }
...
#else
                return System.Activator.CreateInstance(ReflectedType, /*nonPublic:*/ true);
#endif
            }
