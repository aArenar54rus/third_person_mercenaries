...
namespace FullSerializer.Internal {
    public static class fsVersionManager {
        private static readonly Dictionary<Type, fsOption<fsVersionedType>> cache = new Dictionary<Type, fsOption<fsVersionedType>>();
        ...
        public static fsOption<fsVersionedType> GetVersionedType(Type type) {
            ...
            lock (cache)
            {
                if (cache.TryGetValue(type, out optionalVersionedType))
                {
            ...
            if (attr != null && (!string.IsNullOrEmpty(attr.VersionString) || attr.PreviousModels != null)) {
                ...
                lock (cache)
                {
                    cache[type] = optionalVersionedType;
                }
            }
