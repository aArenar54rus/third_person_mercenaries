...
#region Attribute Queries
        ...
        public static Attribute GetAttribute(MemberInfo element, Type attributeType, bool shouldCache) {
            ...
            Attribute attribute;
            lock (cachedAttributeQueries)
            {
                if (!cachedAttributeQueries.TryGetValue(query, out attribute))
                {
                    var attributes = element.GetCustomAttributes(attributeType, /*inherit:*/ true);
                    if (attributes.Length > 0)
                        attribute = (Attribute) attributes[0];
                    if (shouldCache)
                        cachedAttributeQueries[query] = attribute;
                }
            }
        ...
        private static readonly IDictionary<AttributeQuery, Attribute> cachedAttributeQueries =
